...
| 2 | `country.international_employers`, `city.international_employers` | **A `LabelSet` of named firms** — the type section 7.1 declares — user-overridable, and the names are the evidence | `low` | Both |
| 3 | **Match rule research** — visa pathways, quotas, timing (section 7.3) | A proposed result and reason, citing official pages, **always confirmed by a human** before it stands. A proposal is stored, displayed with its sources and **rules nothing out**; confirming it means writing the same answer as `manual`, which replaces it (Q218) | `low` until confirmed | Country |
...
| `country.total_tax_rate_effective` | 30% | **Ratio** — every tax and contribution, employee's and employer's, as a share of total labour cost, at 167% of the average wage | OECD Taxing Wages (`AV_TW`); an estimate from Eurostat's tax-benefit figures, ranked second, for the five EU members OECD does not cover (Q207) | 24 months |
...
| Q218 | **A researched gate is a proposal until a human writes it** (2026-09-12) | Section 6.10 already said "always confirmed by a human"; this is what that means in the data. An answer whose source is a model carries `is_proposal`, derived from the source catalog rather than stored twice, and `gates_that_rule_out` skips it -- so a country is never excluded because a model said so with nobody having looked. Confirming means writing the same answer as `manual`, which replaces the proposal: one row per gate and candidate, so a proposal and a finding can never disagree |
| Q219 | **Prices are required configuration for the LLM path, with no defaults** (2026-09-12) | A run that cannot measure its own cost cannot be capped. A default of zero would report every call as free and make the cap ornamental; a default of a real price would be this application inventing what Anthropic charges. Absent prices mean the path stays off, which the boot log says out loud |
...
| Q225 | **A run is estimated from ceilings, and the one figure that cannot be derived is measured** (2026-09-12) | Section 6.3 asks for a dry-run estimate, and it reported zero for every run -- hardcoded from when every source was free, so the one screen that exists to answer "what will this cost?" answered "nothing", whatever the run was. The estimate now comes from the source itself, because a source is the only thing that knows what it charges. Two of the three figures behind an LLM call's price are ceilings the client already imposes and sends with every request: `max_tokens` caps the output, `max_searches` caps the searches. The input side cannot be known in advance -- the provider injects the pages it searched into the context, so the prompt's own length says little, and the input side dominates the bill. It is therefore **configured and measured** rather than assumed: `make live-llm` makes one real call and prints what it billed, and `LLM_INPUT_TOKENS_PER_CALL` is set from that. **The estimate carries what it rests on** (`estimate_basis`), because a euro figure with no stated assumptions can be trusted but not checked. A figure set too low costs money rather than accuracy, and the cap is what catches it -- the two mechanisms Q22 deliberately kept separate. **Gate research gets its own estimate** (`planMatchRuleResearch`), counting the same pairs the pass would ask through the same function, because it is the most expensive thing this application does and until now the only way to learn the cost was to pay it |
| Q226 | **A second criteria set ships: `remote_only`** (2026-09-13, migration `0469`) | The MVP's own claim, tested: a scenario is **re-weighted rows, not new machinery** (`devplan.md` 8). Nothing measured changes -- the same 43 attributes, the same figures, the same provenance -- and switching sets re-ranks instantly without fetching anything (section 5.6). The scenario: you work remotely for an employer outside the country you live in, so the local job market stops deciding where you may live, and three secondary things become decisive -- whether a tax treaty makes remote work viable at all (economics 20 -> 35), whether the connection is good enough to work over (broadband 25 -> 40), and whether residency is obtainable without a local employer sponsoring you (residency admin 15 -> 25). Career falls from 14% of the level to 4%. **The criteria are copied by SELECT rather than retyped**, because a hand-typed list would silently omit an attribute the day one is added and the omission would read as a deliberate exclusion. Every number is provisional, exactly as Q104's are. It changes the answer: Cyprus rises from 4th to 2nd, Ireland and Liechtenstein leave the top ten |
| Q227 | **Air connectivity splits into two attributes, one European and one worldwide** (2026-09-13, migration `0470`) | `international_air_connectivity` counts "international destinations served", and Eurostat `avia_paocc` -- the only free source that could answer it -- carries a partner list of 35 European countries and three aggregates. No Brazil, no United States, no Gulf hub. A count from it would be European connectivity wearing the word "international", which is the fault Q202 fixed for `crime_safety_index`: a name promising one quantity over a figure measuring another. Two attributes now, each measuring what its name says -- the European one counted from Eurostat, the worldwide one answered by the LLM path with the pages it read. Their weight was split rather than duplicated, because the two overlap and air travel must not count twice in one pillar |
| Q228 | **European air connectivity is descriptive, not scored** (2026-09-13, migration `0471`) | Fetched live for all 31 candidates with an airport, it spans **28 to 34** and takes six distinct values; **Malta ties Germany**. At country granularity within Europe, "can you fly there" is answered yes for almost everyone, and Eurostat's partner list caps the count at 35. Scoring that turns three points of noise into rank differences: under `percentile`, Estonia's 28 against Latvia's 31 becomes a visible connectivity gap that means nothing. **The figure is right and scoring it would be wrong** -- not inaccurate, just not a distinction, which is the plausible-looking number section 10 exists to prevent. So it takes the category section 3.0 already has: an attribute with no criterion attached is descriptive and never scored. It keeps its pillar, its source and its provenance, is fetched and displayed as context, and the worldwide measure takes back the whole air weight. **Found by fetching it rather than by reasoning about it**, which is the argument for a live check on every new adapter |
| Q229 | **The shared vocabulary registry is dropped, and the per-attribute one is finally used** (2026-09-13, migration `0472`) | `label_vocabulary` was created in `0001` to hold *named* vocabularies several attributes could share, and its own comment already pointed at `attribute_allowed_label` for per-attribute ones. That table arrived two migrations later and no two attributes in this catalog share a vocabulary, so the registry sat with no rows, no foreign keys and no reader -- an empty table in the schema diagram reads as a feature somebody forgot rather than one that was superseded. **The half worth recording is the other table**: `attribute_allowed_label` was complete and empty. `catalog.sql` reads it, `Attribute.allowed_labels` carries it, and a `LabelSet` payload holding a label the attribute does not permit is refused -- so the guard existed with nothing to guard, and `country.climate_zone`, whose description is literally "Köppen codes", would have accepted `banana`. It now permits the 20 Köppen codes that occur in Europe: the classification's own set, filtered rather than shortlisted, because a vocabulary too narrow refuses a real figure and the refusal would look like a broken adapter. **Only `climate_zone`** -- `international_employers` is named firms (Q221) and a fixed list would refuse every real answer the LLM path exists to give |
| Q230 | **A publisher's table is transcribed and stored under that publisher** (2026-09-14) | Three attributes are published as one table per edition and through no API: RSF's press freedom index (whose site refuses scripts), EF's English Proficiency Index and MIPEX. The household asked for them to be read in a session rather than paid for per country, and the result is better than the LLM path, not just cheaper: **one table read gives every candidate the same edition and methodology**, where 32 searches can each land on a different year -- and did (P38). Each table is a CSV in `published_tables/tables/`, read strictly at boot, one per publisher and attribute, with the page every figure came from, the period the figure *describes* (the 2026 RSF Index describes 2025; EF EPI 2025 uses 2024 test data), and prose workings where a figure needs a caveat. **Stored under the true publisher** -- `rsf`, `ef_epi`, `mipex`, which the catalog already ranked first -- rather than `manual` (which ranks last and would claim a person typed it) or `llm` (which it is not). The retrieval date is the day the table was read, because a run re-copying an unchanged file has not looked at the publisher again. Refreshing is re-transcribing, which the file's header says how to do. Every transcription was checked against a deterministic read of its source before being kept: RSF 32 of 32, MIPEX's EU table parsed from the page text, EF's 25 scores each from EF's own pages |
| Q231 | **MIPEX: the 2025 edition for the EU, the 2020 edition for Iceland, Norway, Switzerland and the UK** (2026-09-14) | MIPEX 2025 covers only the 27 EU member states and describes 2023; the four non-EU candidates have nothing newer than MIPEX 2020, describing 2019 -- already older than the attribute's five-year `max_age`. The household chose the newest figure everywhere, each carrying its own year, over one stale edition for all 31 or leaving four candidates unscored. **The two editions are on one scale**: the 2025 table prints each score's change since 2019, and every implied 2019 score matches MIPEX 2020. The four older rows show as stale on screen and still count, because nothing fresher exists and freshness orders sources rather than excluding a figure |
