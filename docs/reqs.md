...
| Q200 | **All SQL lives in a top-level `storage/` directory** — migrations and runtime queries alike | SQL is an asset of the system, not a Python implementation detail. Whatever applies it is a detail: a shell script, or something written in another language, must be able to find and run it without knowing how a Python package is laid out internally. The earlier argument for keeping queries inside the package — that it made "storage is the only module that writes SQL" literal — was already false, since 34 migration files sat outside it. The `storage/` **module** is the adapter; the `storage/` **directory** is the asset |
