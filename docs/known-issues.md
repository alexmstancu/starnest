...
**35 findings: 23 closed, 12 open.** **One medium is open**: `POST /v1/data-acquisition-runs`
answers 202 and then blocks, so a client timeout is indistinguishable from a failure and a retry
starts a duplicate run (P35, which cost 2.70 EUR to discover). The rest is documentation,
test-quality and two spend-accounting gaps (P28, P36).
