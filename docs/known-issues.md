...
| **P13** | **One interface test times out under load.** A `ConfigureScreen` test hit its 5-second limit during a `make check` run with containers and a live fetch in flight, and passes in under a second on its own. It bit again at Gate C, where the Configure screen now mounts seven panels behind six requests and typing into one re-renders all of them | **Fixed 2026-09-12.** The tests were already event-driven; the budget was wrong. `testTimeout` is 15 seconds, which still catches a test that will never finish while letting a busy machine be slow rather than red |
...
| **P18** | **A run accounts for fewer items than it has.** A one-item run for Liechtenstein reports `items_total 1, items_completed 0, items_failed 0`: Eurostat answered, the dataset simply has no row for that country, and the adapter records neither a figure nor a failure. The screen then says "Nothing failed in this run" about a run that learned nothing. Run 2 of the real database shows 96 items and 93 completed; run 1 shows 160 and none. **There is no third category** -- `RunProgress` has `items_total`, `items_completed` and `items_failed`, and an item that no source could answer belongs to none of them | **Open -- a contract decision.** Two honest shapes: add `items_unanswered` to `RunProgress`, or record "the source answered, and not about this candidate" as a failure of its own kind. The second makes every sparse indicator produce failures on every run, which is noisy but retryable; the first is quieter and leaves nothing to retry. **Worth Alex's call**, because it decides what a retry is for |
