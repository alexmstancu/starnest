...
- **Interface small change**: Refresh relabels the *Estimate* button; two external scores from one
  ...
  fetches every level's results; the Run-history link is coupled to `ROUTES[1]` positionally;
  comparison keys collide on a comparator-less pair; and `useResource`, the fetch primitive
  everything rests on, has no test of its own.
