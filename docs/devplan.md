...
| Stream | Tab | Size |
|---|---|---|
| **W6-A** | ~~**Configure**~~ **Done 2026-09-12.** Seven panels on one screen, ordered by what depends on what: the household first (gates and defaults read it), then settings, the sets of priorities (create, rename, discard — a new set is empty, because copying one would be deciding for the user), pillar weights, the criteria inside them, the rules the set lets act, and the source priority last, shown and read-only because it is configuration rather than preference. Every rebalance and every refusal is the server's; the panels send one change and render the answer. **Two defects it found: a rebalanced pillar weight did not reach the inputs that had moved, and a panel carried one set's weights and rules into the next** | **L** |
| **W6-B** | ~~**Rank**~~ **Done 2026-09-12.** The table already kept non-matching candidates in place with their scores; it now shows how much of each score rests on low-confidence figures, and any warning a rule raised. Opening a candidate shows **every stored value** -- the one being scored, the ones it supersedes and any rejected, each with its source, both dates and its confidence -- and the outside indices in their own bordered table, never among the figures. `GET /external-scores` was built for it (30 of 40 operations) | **L** |
...
| Stream | Task | Size |
|---|---|---|
| **W7-A** | ~~The **LLM path**~~ **Done 2026-09-12.** All three permitted country-level uses (`reqs.md` 6.10), each priced and each required to cite: named employers as a `LabelSet`, a fallback figure for an attribute no dataset covers, and gate research whose answers are **proposals** that rule nothing out until a human writes them (Q218). `CostMeter`, the cap and its halt; **a run that can spend refuses without a cap and the refusal can be accepted past** (Q220); **a paid source is asked only when a run names its attributes**, so a sweep is free by construction. Prices are required configuration with no defaults (Q219). Tested against recorded shapes, with one opt-in live call behind `make live-llm` -- the only thing here that spends. Original scope: The **LLM path** — the Anthropic SDK with `web_search`, as a `SourceAdapter` like any other. `CostMeter`, the spend cap and its halt, and **storing the pages the model actually read**. Country level uses it for `international_employers` only. **The inventory in `reqs.md` 6.10 is exhaustive** — an unlisted use is a section 0.3 stop | **M** |
| **W7-B** | ~~Operations~~ **Partly done 2026-09-12**: the startup sequence refuses to start against a schema the code is ahead of, validates every adapter declaration against the catalog, sweeps the runs a dead process left behind, and logs each decision; the run lifecycle, adapter failures and spend accumulation are logged; `pg_dump` before every migration and `log_statement = 'mod'` were already in place; the API key reaches no log, row or error message, and a test asserts it. Original scope: Operations — the startup sequence and its **refusal to start on a schema gap**, adapter-declaration validation at boot, the abandoned-run sweep, `pg_dump` before every migration, `log_statement = 'mod'`, application logging, and the assertion that **the API key never reaches the log, the database or an error message** | **M** |
