...
research whose answers are **proposals that rule nothing out until a human writes them** (Q218), now with a screen that lists them with their pages and confirms one.
...
| `docs/openapi.yaml` | **Written.** The REST contract — 42 operations, **all of them served**; the target and the generated truth have been in exact step since 2026-09-12. It stays the target rather than a record of the code: it is written first, and `docs/openapi.implemented.yaml` is generated from the routes. The **target**, hand-written and deliberately ahead of the code. `docs/openapi.implemented.yaml` is the generated **truth**; see "Two contracts" below |
